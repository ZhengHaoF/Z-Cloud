...
function add_file_key_in_db($conn,$user_name, $file_name,$file_key ,$file_size)
{
    //添加新的文件参数在数据库中
    if(select_db_have_file_key($conn,$file_key)==1){
        ...
        if (mysqli_fetch_row($res)[0]==0){
            ...
            //添加进用户文件表
            $res = mysqli_query($conn, "INSERT INTO user_files VALUES('$user_name','$file_name','$file_key','$file_size')");
            return "引用数+1";
        }else{
            $res = mysqli_query($conn, "INSERT INTO user_files VALUES('$user_name','$file_name','$file_key','$file_size')");
            return "文件已存在";
